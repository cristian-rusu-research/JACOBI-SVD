function [positions1, values1, values2, Ubar, approx_error, tus] = svd_approximation_two_side(U, p, g)
...
%         scores_nuclear(i, j) = sqrt(c1 + sqrt(c1_2 - c2_2)) + sqrt(c1 - sqrt(c1_2 - c2_2)) - trace(T);
        ...
        if U(i,i)*ujj - U(i,j)*uji >= 0
            scores_nuclear(i, j) = sqrt( (U(i,i) + ujj)^2 + (U(i,j) - uji)^2 ) - U(i,i) - ujj;
        else
            scores_nuclear(i, j) = sqrt( (U(i,i) - ujj)^2 + (U(i,j) + uji)^2 ) - U(i,i) - ujj;
        end
    end
